...
def get_args():
    parser = argparse.ArgumentParser(description="Evaluate CoralSRT")
...
def main():
    ...
    save_dir = os.path.join("D:/Coral_SRT/results", args.model_name, f"{args.num_points}_points")
    ...
    if os.path.exists(args.checkpoint_path):
        ...
        elif isinstance(checkpoint, dict) and 'denoiser' in checkpoint: # Support checkpoint gốc tác giả
    ...
    for feat_name in tqdm(feat_files):
        ...
        rec_feat_upsampled = F.interpolate(rec_feat_tensor, size=(H_orig,W_orig),mode='bilinear',align_corners=False).squeeze(0)
        ...
        save_colored_mask(pred_mask_orig.cpu().numpy(), save_path)
